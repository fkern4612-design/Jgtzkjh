...
@app.route("/health")
def health():
    running = _scheduler_started and (_scheduler_thread is not None and _scheduler_thread.is_alive())
    return jsonify({"status": "ok", "scheduler_running": running})


@app.route("/")
def index():
    return "TikTok Continuous Runner is running. Check /health. POST /start to start scheduler in this worker."
